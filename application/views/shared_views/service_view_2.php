...
function get_oas_bget($orParams, $andParams) {
    ...
    foreach ($resultAnd as $key) {
        $arregloAnd[$key['rep_id'].'-'.$key['lo_id']], $key['lo_xml_lom'];
    }
    $evitDatRep=true;
    ...
    //si ya lo mostro entonces no se vuelve a mostrar
       foreach ($resultAnd as $mostrados ) {
        ...
        if($evitDatRep){
            $arregloOr[$key['rep_id'].'-'.$key['lo_id']], $key['lo_xml_lom'];
        }
    }
    
    //var_dump($arregloAnd);
    
    /*foreach($arregloAnd as $key => $value){
        $result[$key] = $value;
    }
    
    foreach($arregloOr as $key => $value){
        $result[$key] = $value;
    }*/
